...
import VideoLessons from './components/VideoLessons'; // Yangi
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const storageKey = `ld_blogs_${lang}`;
    const saved = localStorage.getItem(storageKey);
    if (saved) {
      try { setBlogs(JSON.parse(saved)); } catch (e) { setBlogs(MOCK_BLOGS_BY_LANG[lang] || MOCK_BLOGS_BY_LANG['en']); }
    } else {
      setBlogs(MOCK_BLOGS_BY_LANG[lang] || MOCK_BLOGS_BY_LANG['en']);
    }
    const savedCats = localStorage.getItem('ld_categories');
  ...
  // Fix: Adding handleUpdatePost to update the blogs state when a post is modified in the AdminPanel
        ...
        {viewMode === 'about' ? (
          ...
          <div className="flex flex-col lg:flex-row gap-12 lg:gap-16">
            <div className="order-2 lg:order-1 lg:w-80 shrink-0">
               <Sidebar categories={categories} activeCategory={activeCategory} onSelectCategory={setActiveCategory} t={t} />
               <button onClick={() => setIsAdminOpen(true)} className="mt-10 w-full flex items-center justify-center gap-3 p-6 glass rounded-[2.5rem] text-slate-500 font-black uppercase tracking-widest text-[10px] hover:text-slate-950 dark:hover:text-white transition-all group shadow-xl hover:shadow-2xl">
            ...
            <div className="flex-1 order-1 lg:order-2 space-y-12">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                {filteredBlogs.map((post) => (
                  <article key={post.id} onClick={() => handlePostClick(post.id)} className="group cursor-pointer glass rounded-[3rem] overflow-hidden transition-all duration-500 hover:scale-[1.02] hover:shadow-2xl border border-white/40 dark:border-white/5">
                    ...
                    <div className="p-10">
                      ...
                      <h3 className="text-2xl font-black mb-4 dark:text-white leading-tight group-hover:text-blue-500 transition-colors">{post.title}</h3>
